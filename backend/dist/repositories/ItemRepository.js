...
class ItemRepository extends BaseRepository_1.BaseRepository {
  ...
  `;
    ...
    async searchItems(filters, pagination, userLat, userLng) {
        ...
        const conditions = ['i.is_active = true', 'i.is_available = true'];
        const whereParams = [];
        const distanceParams = [];
        const searchSelectParams = [];
        const searchWhereParams = [];
        ...
        if (filters.search) {
            ...
            conditions.push('(MATCH(i.title, i.description) AGAINST(? IN NATURAL LANGUAGE MODE) OR i.title LIKE ? OR i.description LIKE ?)');
    ...
    `;
        ...
        const rows = await (0, database_1.executeQuery)(dataQuery, dataParams);
    ...
    `;
        return (0, database_1.executeQuery)(query, [itemId]);
    }
    // Add item image
    async addImage(itemId, url, altText, isPrimary = false) {
        const id = this.generateId();
        // If this is primary, unset other primary images
        if (isPrimary) {
            await (0, database_1.executeQuery)('UPDATE item_images SET is_primary = false WHERE item_id = ?', [itemId]);
        }
        const query = `
    ...
    `;
        await (0, database_1.executeQuery)(query, [id, itemId, url, altText, isPrimary, itemId]);
    ...
    async removeImage(imageId) {
        ...
        const result = await (0, database_1.executeQuery)(query, [imageId]);
    ...
    async findNearby(lat, lng, radiusKm = 10, limit = 20) {
      ...
      LIMIT ?
    `;
        const rows = await (0, database_1.executeQuery)(query, [lat, lng, lat, radiusKm, limit]);
        const items = [];
        for (const row of rows) {
            const images = await this.getItemImages(row.id);
            items.push({
    ...
    async isOwner(itemId, userId) {
        ...
        const rows = await (0, database_1.executeQuery)(query, [itemId, userId]);
        return rows.length > 0;
    }
    // Set item availability
    async setAvailability(id, isAvailable) {
        const query = 'UPDATE items SET is_available = ? WHERE id = ?';
        const result = await (0, database_1.executeQuery)(query, [isAvailable, id]);
        return result.affectedRows > 0;
    }
}
