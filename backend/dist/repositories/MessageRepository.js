...
class MessageRepository extends BaseRepository_1.BaseRepository {
  ...
  `;
    ...
    async getConversation(userId1, userId2, limit = 50) {
      ...
      LIMIT ?
    `;
        const rows = await (0, database_1.executeQuery)(query, [userId1, userId2, userId2, userId1, limit]);
        return rows
            .map((row) => ({
            id: row.id,
            sender_id: row.sender_id,
    ...
    async markAsRead(senderId, recipientId) {
        const query = 'UPDATE messages SET is_read = true WHERE sender_id = ? AND recipient_id = ? AND is_read = false';
        const result = await (0, database_1.executeQuery)(query, [senderId, recipientId]);
        return result.affectedRows;
    }
    // Mark specific message as read
    async markMessageAsRead(messageId) {
        const query = 'UPDATE messages SET is_read = true WHERE id = ?';
        const result = await (0, database_1.executeQuery)(query, [messageId]);
        return result.affectedRows > 0;
    }
    // Get unread message count for user
    async getUnreadCount(userId) {
        const query = 'SELECT COUNT(*) as count FROM messages WHERE recipient_id = ? AND is_read = false';
    ...
    `;
        ...
        const rows = await (0, database_1.executeQuery)(query, [userId, userId, searchPattern, limit]);
        return rows.map((row) => ({
            id: row.id,
            sender_id: row.sender_id,
            recipient_id: row.recipient_id,
