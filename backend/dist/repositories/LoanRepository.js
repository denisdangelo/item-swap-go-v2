...
class LoanRepository extends BaseRepository_1.BaseRepository {
  ...
  `;
    ...
    async create(data, borrowerId, dailyRate) {
        ...
        const itemResult = await (0, database_1.executeQuery)(itemQuery, [data.item_id]);
    ...
    `;
        const images = await (0, database_1.executeQuery)(imagesQuery, [
            row.item_id,
        ]);
        return {
            id: row.id,
    ...
    `;
        const rows = await (0, database_1.executeQuery)(query, [userId, userId]);
        const loans = [];
    ...
    async hasConflictingLoans(itemId, startDate, endDate, excludeLoanId) {
        let query = `
      SELECT 1 FROM loans
      WHERE item_id = ? 
