...
exports.AzureSchemaMigrator = void 0;
...
exports.AzureSchemaMigrator = AzureSchemaMigrator;
{
    users: { }
    categories: { }
    items: { }
...
Promise < void  > {
    ...
    this: .sourceConnection = await promise_1.default.createConnection(this.config.source),
    this: .targetConnection = await promise_1.default.createConnection(this.config.target),
...
Promise < void  > {
    ...
    for(, table, of, tables) {
        const [sourceCount] = await this.sourceConnection.execute(`SELECT COUNT(*) as count FROM ${table.source}`);
        const [targetCount] = await this.targetConnection.execute(`SELECT COUNT(*) as count FROM ${table.name}`);
