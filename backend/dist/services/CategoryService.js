...
class CategoryService {
    ...
    async createCategory(categoryData) {
        ...
        if (existingCategory) {
            throw new errorHandler_1.ConflictError('Category with this name already exists');
        }
    ...
    async updateCategory(id, updateData) {
        ...
        if (!updatedCategory) {
            throw new errorHandler_1.NotFoundError('Failed to update category');
        }
    ...
    async deleteCategory(id) {
        ...
        if (isInUse) {
            throw new errorHandler_1.ConflictError('Cannot delete category that is being used by items');
        }
    ...
    async deactivateCategory(id) {
        ...
        if (!category.is_active) {
            throw new errorHandler_1.ConflictError('Category is already deactivated');
        }
    ...
    async activateCategory(id) {
        ...
        if (category.is_active) {
            throw new errorHandler_1.ConflictError('Category is already active');
        }
    ...
    validateCategoryData(data) {
        if ('name' in data && data.name) {
            if (data.name.trim().length < 2) {
                throw new errorHandler_1.ValidationError('Category name must be at least 2 characters long');
            }
            if (data.name.trim().length > 100) {
                throw new errorHandler_1.ValidationError('Category name must not exceed 100 characters');
            }
        }
        if ('description' in data && data.description) {
            if (data.description.trim().length > 1000) {
                throw new errorHandler_1.ValidationError('Category description must not exceed 1000 characters');
            }
        }
        if ('color' in data && data.color) {
            ...
            if (!hexColorRegex.test(data.color)) {
                throw new errorHandler_1.ValidationError('Color must be a valid hex color (e.g., #FF0000)');
            }
        }
        if ('icon' in data && data.icon) {
            if (data.icon.trim().length > 100) {
                throw new errorHandler_1.ValidationError('Icon name must not exceed 100 characters');
            }
        }
    }
}
